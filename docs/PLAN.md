...
- [x] **Cihaz videosu düzeltmeleri (kullanıcı geri bildirimi):** (1) rota/menü geçişinde "ortada balon → zıplama" sıçraması giderildi — geçiş sürerken yalnızca karartma görünür, balon+halka hedef ölçülünce birlikte belirir (`hazir` durumu); (2) balon artık güvenli alan içinde konumlanır (çentik/ana çubuk altını ölçüp `env(safe-area-inset-*)` ile kırpar) — eskiden aşağıdaki hedeflerde balon durum çubuğunun/saatin arkasına kaçıyordu; (3) yumuşak beliriş + öğeler arası kayma animasyonu (reduced-motion'da kapalı); (4) metinler **senaryo temelli** yeniden yazıldı ("bir cevap dilekçesi hazırlamanız gerekiyor → menüden Asistan'a → gerçek panel") — kullanıcının istediği "senaryoya göre eğitim". Tarayıcıda doğrulandı: 8 durakta sıçrama yok, balonlar ekran içinde, gerçek paneller spotlight.
